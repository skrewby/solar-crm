...
const RoofTypeOptions = () => {
  ...
  const getData = useCallback(async () => {
    ...
    try {
      const result = await bpmAPI.getRoofTypes();
      if (result.data) {
        setData(result.data);
      }
    } catch (err) {
  ...
  const updateMyData = async (rowIndex, columnId, value) => {
    ...
    const result = await bpmAPI.updateRoofType(newData[rowIndex].id, newData[rowIndex]);
    if (result.data) {
      setData(newData);
    }
  };
  ...
  return (
    <MainCard title="Roof Types" content={false}>
...
export default RoofTypeOptions;
