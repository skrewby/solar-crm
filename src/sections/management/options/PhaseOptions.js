...
import AddPhaseForm from './forms/AddPhaseForm';
...
const PhaseOptions = () => {
  ...
  const getData = useCallback(async () => {
    ...
    try {
      const result = await bpmAPI.getPhases();
      if (result.data) {
        setData(result.data);
      }
    } catch (err) {
  ...
  const updateMyData = async (rowIndex, columnId, value) => {
    ...
    const result = await bpmAPI.updatePhase(newData[rowIndex].id, newData[rowIndex]);
    if (result.data) {
      setData(newData);
    }
  };
  ...
  return (
    <MainCard title="Phases" content={false}>
      ...
      <AddPhaseForm onFormSubmit={onAddOption} openDialog={openDialog} setOpenDialog={setOpenDialog} />
    </MainCard>
  );
};
